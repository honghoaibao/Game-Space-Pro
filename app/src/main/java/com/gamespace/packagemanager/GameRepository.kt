...
class GameRepository @Inject constructor(
    ...
    /**
     * Danh sách app có thể chọn khi thêm game thủ công (dialog "Chọn ứng dụng"), thay cho
     * việc bắt người dùng tự gõ package name. Chỉ liệt kê app có icon trên màn hình chính
     * (`CATEGORY_LAUNCHER`) — không liệt kê toàn bộ ~300+ package hệ thống không có UI,
     * vừa đúng ý định người dùng (chọn app để chơi) vừa tránh danh sách dài không cần thiết.
     */
    suspend fun queryAllLaunchableApps(): List<InstalledAppInfo> = withContext(Dispatchers.IO) {
        ...
        resolveInfos
            ...
            .distinct()
            .filterNot { it == context.packageName } // không tự liệt kê chính GAME SPACE
            .mapNotNull { pkg ->
                runCatching {
                    val appInfo = packageManager.getApplicationInfo(pkg, 0)
                    InstalledAppInfo(packageName = pkg, appLabel = appInfo.loadLabel(packageManager).toString())
                }.getOrNull()
            }
