...
class GameDetectionService : Service() {
    ...
    private fun buildNotification(activePackage: String?): Notification =
        NotificationCompat.Builder(this, GameSpaceApp.CHANNEL_AUTOMATION)
            ...
            .setOngoing(true)
            // Xem ghi chú ở OverlayService.buildNotification() / GameSpaceApp.createNotificationChannels() —
            // service này quyết định Overlay HUD có được mở lại khi vào game hay không, nên cũng cần
            // được hệ thống coi là ưu tiên, không bị dọn nền như service phụ.
