...
class HardwareDetector @Inject constructor(
    ...
    /**
     * `detect()` chạy NGAY ở cold start (`DashboardViewModel.init`) — đây từng là nguyên nhân
     * app crash ngay khi mở (`readRefreshRates()` gọi `WindowManager.getDefaultDisplay()` ném
     * `UnsupportedOperationException` trên API 30+, xem KDoc trong hàm đó). Bọc try/catch ở
     * điểm vào công khai duy nhất của class này làm lớp phòng thủ cuối: nếu một máy/OEM hiếm
     * nào đó ném lỗi tương tự ở một API khác trong tương lai, Dashboard chỉ mất phần Device
     * Info hiển thị (fallback rỗng, an toàn) thay vì crash toàn app.
     */
    fun detect(): DeviceInfo = try {
        detectInternal()
    } catch (t: Throwable) {
        fallbackDeviceInfo()
    }

    private fun detectInternal(): DeviceInfo {
    ...
    /** Dùng khi [detectInternal] ném lỗi không lường trước — chỉ giữ lại thông tin không bao giờ crash. */
    private fun fallbackDeviceInfo(): DeviceInfo = DeviceInfo(
        manufacturer = Build.MANUFACTURER,
        model = Build.MODEL,
        androidVersion = Build.VERSION.RELEASE.orEmpty(),
        apiLevel = Build.VERSION.SDK_INT,
