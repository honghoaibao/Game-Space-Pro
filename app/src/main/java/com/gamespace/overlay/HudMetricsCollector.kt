 ...
 *
 * CPU% chưa đọc được (cần `/proc/stat` qua Shizuku, để lại cho việc tối ưu sau) —
 * giá trị `null` được [HudMetrics] xử lý bằng cách ẩn dòng CPU trên UI.
 */
@Singleton
class HudMetricsCollector @Inject constructor(
    ...
    private suspend fun buildMetrics(fps: Int): HudMetrics {
        ...
        return HudMetrics(
            ...
            thermalLabel = thermalMonitor.label(thermalMonitor.currentStatus()),
