...
| 21 | Fix crash mở app + Thêm game qua danh sách chọn + Splash screen | ✅ Xong (Phiên 8) | `HardwareDetector` dùng `DisplayManager` thay `WindowManager.defaultDisplay()` (+ try/catch phòng thủ), `applicationScope` có `CoroutineExceptionHandler`; `GameLibraryScreen` đổi dialog "gõ package name" thành dialog chọn từ danh sách app đã cài (có tìm kiếm); thêm splash screen (`androidx.core.splashscreen`) với hiệu ứng thoát tùy chỉnh |
