...
| 21 | Fix crash mở app + Thêm game qua danh sách chọn (thay gõ package name) | ✅ Xong (Phiên 8) | `HardwareDetector` dùng `DisplayManager` thay `WindowManager.defaultDisplay()`; `GameLibraryScreen` đổi dialog "gõ package name" thành dialog chọn từ danh sách app đã cài (có tìm kiếm) |
