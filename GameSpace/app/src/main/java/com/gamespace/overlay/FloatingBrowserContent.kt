...
private fun BrowserBubble(onClick: () -> Unit, onDrag: (Float, Float) -> Unit) {
    ...
    Surface(
        ...
        modifier = dragModifier.size(52.dp),
        onClick = onClick,
    ) {
        Box(contentAlignment = Alignment.Center) {
            Icon(Icons.Filled.Public, contentDescription = "Mở trình duyệt nổi", tint = Color.White)
        }
    }
}
...
private fun BrowserPanel(
    ...
    Surface(
        ...
        color = Color(0xF0101826),
        contentColor = Color.White,
        modifier = Modifier.fillMaxSize(),
    ) {
        Box(Modifier.fillMaxSize()) {
            Column(Modifier.fillMaxSize()) {
                // Title bar — kéo để di chuyển cả cửa sổ (trừ khi đã khóa vị trí).
                Row(
                    modifier = dragModifier.fillMaxWidth().padding(6.dp),
                ...
                Row(
                    ...
                    IconButton(onClick = { onNavigate(state.addressBarText) }, modifier = Modifier.size(32.dp)) {
                        Text("Đi", color = Color.White)
                    }
                }
...
private fun TouchGuardOverlay(onUnlock: () -> Unit) {
    Box(
        ...
        Surface(shape = RoundedCornerShape(8.dp), color = Color(0xCC000000), onClick = onUnlock) {
            Row(Modifier.padding(12.dp, 8.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Text("Chạm để mở khóa thao tác", color = Color.White, style = MaterialTheme.typography.labelSmall)
            }
        }
    }
}
