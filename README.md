...
- **Kotlin 2.3.20**, not 2.2.20 as originally pinned. The original choice
  (2.2.20, "a version I have high confidence writing correct syntax for")
  turned out to be wrong in a way I only found via a real CI failure:
  "Could not find any convention object of type KotlinSourceSet" applying
  `org.jetbrains.kotlin.jvm`. Root cause — confirmed against kotlinlang.org's
  own compatibility tables, not guessed — is that Kotlin 2.2.20's
  fully-supported Gradle range doesn't reach this project's pinned Gradle
  8.14 (2.1.20's documented range is "7.6.3 through 8.11"; 2.2.20 isn't
  much wider), while **2.3.20 is explicitly documented as "fully compatible
  with Gradle 7.6.3 through 9.3.0."** Confidence-in-syntax isn't worth much
  if the version doesn't actually run — see `libs.versions.toml`'s comment
  for the full chain of evidence. KSP moved to `2.3.9` alongside this —
  see that comment too, KSP's versioning scheme itself changed in a way
  worth understanding before touching either version again.
...
referencing tasks that don't exist would fail on its first run. **Each
module applies both plugins itself**, in its own `plugins { }` block
(`alias(libs.plugins.ktlint)` / `alias(libs.plugins.detekt)`) — not via a
single `subprojects { apply(plugin = ...) }` call in the root script,
which is what an earlier version of this file did. That centralized
approach caused a real, repeatable CI failure ("Could not find any
convention object of type KotlinSourceSet" applying
`org.jetbrains.kotlin.jvm` to `:domain`) that persisted across two
different Kotlin versions — ruling out a version-compatibility
explanation and pointing instead at ktlint's plugin reacting to
`subprojects { }`'s cross-project timing in a way it doesn't when applied
from a module's own `plugins { }` block the normal way. The root
`build.gradle.kts` still centralizes the *configuration* (`KtlintExtension`/
`DetektExtension` settings, the `detektPlugins` dependency) via
`subprojects { }`, guarded with `plugins.withId(...)` — that part is pure
configuration of an already-applied plugin, not plugin application, so it
doesn't carry the same ordering risk. Yes, this means 8 modules each
repeat 2 lines in their `plugins { }` block — a `buildSrc` convention
plugin would remove that duplication, and is a reasonable next cleanup
once the build is confirmed green, but wasn't worth the added complexity
while still chasing this specific failure. Detekt's config
